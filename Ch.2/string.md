...
아래 코드를 실행해봅시다. 만약 양쪽 끝의 `"` 를 `'`로 바꾸면 어떻게 될까요?
...
다음은 한 학생의 정보가 담긴 문자열입니다. 슬라이싱을 사용하여 정보를 추출하고 출력하세요.

- 데이터: student_info = "202510456-MSE-Kim"
- 추출 내용
1. 학번 : 202510456
2. 이름 : Kim
3. 전공 : MSE
...
```python
student_info = "202510456-MSE-Kim"
...
print(f"학번: {student_id}, 이름: {name}, 전공: {major}")
```

### **2. 문자열 뒤집기 (Slicing 응용)**

사용자로부터 임의의 단어를 입력받아, 그 단어를 거꾸로(뒤에서부터) 출력하는 코드를 작성하세요.

- 힌트 : Slicing의 step 옵션을 활용해보세요!

```python
word = input("뒤집을 단어를 입력하세요: ")

# 슬라이싱을 활용하여 거꾸로 출력
reversed_word = 
print(f"입력한 단어의 역순: {reversed_word}")
```

### **3. 포매팅을 활용한 데이터 출력**

아래와 같은 실행 결과가 나오도록 포매팅을 사용하여 코드를 작성하세요.
위에서 배운 포매팅 방법 중 2-4번 방법 3가지로 같은 내용 3줄을 출력하세요.  

- 출력 결과 : **주문하신 Coffee는 4500원입니다.**
...
원주율($\pi$) 값을 소수점 셋째 자리까지 출력하고, 전체 길이를 10으로 하되 오른쪽 정렬하여 출력하세요.

- 출력 결과 : 원주율 값 출력: '&nbsp;&nbsp;&nbsp;&nbsp;&nbsp;3.142'

```python
import math
pi = math.pi

# 원하는 포매팅 기법을 사용하여 출력하세요. (전체 10자리, 소수점 3자리, 오른쪽 정렬)
print(                )
```

### **5. 로그 데이터에서 정보 추출하기**

실제 서버 로그나 장비 데이터는 여러 구분자가 섞여 있는 경우가 많습니다. split()을 연속해서 사용하여 특정 정보만 뽑아보세요.

- 데이터: log = "2026-03-18:SUCCESS:ID_105"
- 미션: 로그에서 **날짜**와 **아이디(105)**만 숫자 형태로 추출하여 출력하세요.

```python
log = "2026-03-18:SUCCESS:ID_105"
...
print(f"날짜: {date}, 사용자 ID: {user_id}")
```
