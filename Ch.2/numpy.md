...
|**함수**|**특징**|**예시**|
|:-------|:-------|:-------|:-------|
|`np.arange(start, stop, step)`|시작부터 끝(미포함)까지 간격을 기준으로 생성 |`np.arange(1, 7, 2)` -> `[1, 3, 5]`|
|`np.linspace(start, stop, num)`|시작부터 끝(포함)까지 개수를 기준으로 생성|`np.linspace(0, 10, 5)` -> `[0, 2.5, 5, 7.5, 10]`
