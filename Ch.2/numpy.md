...
|**함수**|**특징**|**예시**|**결과**|
|:-------|:-------|:-------|:-------|
|`np.arange(a, b, x)`|시작(a)부터 끝(b, 미포함)까지 간격(x)을 기준으로 생성 |`np.arange(1, 7, 2)` | `[1, 3, 5]`|
|`np.linspace(a, b, n)`|시작(a)부터 끝(b, 포함)까지 개수(n)를 기준으로 생성|`np.linspace(0, 10, 5)` | `[0, 2.5, 5, 7.5, 10]`|
