...
`toeic.txt`를 읽으면서 각 학과별로 **750점 이상**인 학생 수를 세어 출력하는 함수를 작성합니다.
...
> {: .result .fs-3 }  
> 기계공학과 750점 이상 학생: 6명  
> 산업경영공학과 750점 이상 학생: 30명  
> 원자력공학과 750점 이상 학생: 15명  
> 화학공학과 750점 이상 학생: 22명  
> 신소재공학과 750점 이상 학생: 36명  
> 사회기반시스템공학과 750점 이상 학생: 8명  
> 건축공학과 750점 이상 학생: 15명  
환경학및환경공학과 750점 이상 학생: 30명  
건축학과 750점 이상 학생: 20명  
...
def count_pass(filename):
    ...
    for i in range(9):
        ...
        print(f'{dept} 750점 이상 학생: {count}명')
        total += count

    f.close()
    print(f'경희대학교 공과대학 750점 이상 학생 총 {total}명')
    return dept_names
...
```
(50, 9)
[[720 800 570 745 835 525 605 780 570]
 [655 730 670 755 855 740 635 900 670]
 [735 700 675 650 910 860 765 835 730]]
```
